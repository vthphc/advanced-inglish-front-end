...
async function onSubmit() {
    try {
        ...
        try {
            const apiPath =
                testStore.getSelectedTestType === "Writing"
                    ? `/tests/${id}/submitWriting`
                    : `/tests/${id}/submit`;

            const response = await api.post(apiPath, submissionData);
        ...
        >
            <div class="flex space-x-2 mb-4 border-b">
                <button
                    ...
                    @click="selectLesson(index, lesson.title)"
                >
                    {{ lesson.title }}
                </button>
            </div>
            <div class="flex flex-col lg:flex-row gap-4">
                <div
                    v-if="testStore.getSelectedTestType === 'ReadingListening'"
                    class="flex-1"
                >
                    <div
                        v-for="(lesson, index) in selectedLessons"
                ...
                <div
                    v-if="testStore.getSelectedTestType === 'Writing'"
                    class="flex-1"
                >
                    <div
                        v-for="(lesson, index) in selectedLessons"
                        ...
                        :key="index"
                    >
                        <WritingInstructions :part="currentPartName" />
                        <WritingQuestion
                            v-for="(question, qIndex) in lesson.questionsList"
                            :key="qIndex"
                            :question="question"
                ...
                <div
                    v-if="testStore.getSelectedTestType === 'Speaking'"
                    class="flex-1"
                >
                    <div
                        v-for="(lesson, index) in selectedLessons"
                    ...
                    <div class="space-y-4 flex-1">
                        ...
                        >
                            <Card
                                ...
                                @click="selectLesson(lessonIndex, lesson.title)"
                            >
