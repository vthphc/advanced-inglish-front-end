<script setup lang="ts">
	...
	onMounted(async () => {
		try {
			testData.value = await api.get(
				`/auth/taken-test/${testId}`
			);
			console.log("Raw test data:", testData.value);
			if (testData.value?.takenTest?.lessons) {
				lessonIds.value =
					testData.value.takenTest.lessons.map(
	...
	function getTestType(lesson: Lesson) {
		return (
			lesson.questions[0]?.question?.type ||
			"ReadingListening"
		);
	}
</script>
